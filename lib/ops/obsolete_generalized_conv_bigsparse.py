...
This implementation: Construct a single sparse tensor that encodes the adjacency, which is later split
into layers and then matmul'ed with the [batch, sensors, colors] tensor for each layer. The result
is concatenated and returned. Extremely slow.
"""
...
class AddWeight(object):
    def __init__(self, name, wnames, nout, nin):
        ...
        self.out_index = 0
        self.in_layer_offset = 0
        self.out_layer_offset = 0

        self.indices = dict((wn, []) for wn in wnames)

    def add(self, idx, wname):
        index = (
            self.out_index + self.out_layer_offset,
            idx + self.in_layer_offset
        )
        self.indices[wname].append(index)

    def generate(self):
        dense_shape = [self.out_layer_offset * self.nout, self.in_layer_offset * self.nin]
...
def nearest_neighbor_conv(input, layer_conf, nout, name):
    ...
    kernel = nearest_neighbor_conv_kernel(input, layer_conf, nout, name)
...
def pooling_conv_kernel(input, layer_conf, nout, name):
    ...
    adder = AddWeight(name, wnames, nout, nin)
    ...
    return adder.generate(), out_layer_conf
